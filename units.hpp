...
#define SU_MUL(lhs_1, lhs_2, rhs) \
    ...
    {                             \
        template <>               \
        struct div<rhs, lhs_1>    \
        {                         \
            using type = lhs_2;   \
        };                        \
        template <>               \
        struct div<rhs, lhs_2>    \
        {                         \
            using type = lhs_1;   \
        };                        \
        template <>               \
        struct mul<lhs_1, lhs_2>  \
        {                         \
            using type = rhs;     \
        };                        \
        template <>               \
        struct mul<lhs_2, lhs_1>  \
        {                         \
            using type = rhs;     \
        };                        \
    }
...
#define SU_UNIT(name, symbol_str)                  \
    struct name                                    \
    {                                              \
        static constexpr auto symbol = symbol_str; \
    };
...
namespace su
{
    ...
    public:
        ...
        constexpr Rep count() const { return m_val; }
    ...
    constexpr auto operator*(const unit<Tag1, Rep1, Scale1>& a, const unit<Tag2, Rep2, Scale2>& b)
    {
        ...
        if constexpr (std::is_void_v<typename ops::mul<Tag1, Tag2>::type>) {
            ...
            return unit<typename ops::mul<Tag1, Tag2>::type, std::common_type_t<Rep1, Rep2>,
                        typename std::ratio_multiply<Scale1, Scale2>::type>(v);
        }
    }
    ...
    constexpr auto operator/(const unit<Tag1, Rep1, Scale1>& a, const unit<Tag2, Rep2, Scale2>& b)
    {
        if constexpr (std::is_void_v<typename ops::div<Tag1, Tag2>::type>) {
            ...
            std::common_type_t<Rep1, Rep2> v = a.count() / b.count();
            return unit<typename ops::div<Tag1, Tag2>::type, std::common_type_t<Rep1, Rep2>,
                        typename std::ratio_divide<Scale1, Scale2>::type>(v);
        }
    }
    ...
    std::ostream& operator<<(std::ostream& s, const unit<Tag, Rep, Scale>& u)
    {
        ...
        if constexpr (std::is_same_v<Scale, std::exa>) {
            s << "E";
        } else if constexpr (std::is_same_v<Scale, std::peta>) {
            s << "P";
        } else if constexpr (std::is_same_v<Scale, std::tera>) {
            s << "T";
        } else if constexpr (std::is_same_v<Scale, std::giga>) {
            s << "G";
        } else if constexpr (std::is_same_v<Scale, std::mega>) {
            s << "M";
        } else if constexpr (std::is_same_v<Scale, std::kilo>) {
            s << "k";
        } else if constexpr (std::is_same_v<Scale, std::ratio<1>>) {
            // No prefix
        } else if constexpr (std::is_same_v<Scale, std::milli>) {
            s << "m";
        } else if constexpr (std::is_same_v<Scale, std::micro>) {
            s << "μ";
        } else if constexpr (std::is_same_v<Scale, std::nano>) {
            s << "n";
        } else if constexpr (std::is_same_v<Scale, std::pico>) {
            s << "p";
        } else if constexpr (std::is_same_v<Scale, std::femto>) {
            s << "f";
        } else if constexpr (std::is_same_v<Scale, std::atto>) {
            s << "a";
        } else if constexpr (Scale::den == 1) {
            s << "[" << Scale::num << "]";
        } else {
            s << "[" << Scale::num << "/" << Scale::den << "]";
        }
